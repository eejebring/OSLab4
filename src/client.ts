...
document.addEventListener("DOMContentLoaded", () => {
	...
	for (let group of buttonGroups) {
		const lightId = parseInt(group.dataset.lightId!)
		...
		blinkButton.addEventListener("click", () => {
			blink(lightId, blinkButton, stopBlinkButton)
		})
	...
	function blink(lightId: number, blinkButton: HTMLButtonElement, stopBlinkButton: HTMLButtonElement) {
		fetch(href + apiWebRoot + "/blink/" + lightId, {
			method: "POST"
		}).then((response) => {
			if (response.status === 200) {
